...
async def main():
    ...
    # TODO: ClassType.InvokeMethod - System.loadLibrary("frida-gadget")

    """
    1. JDWP: connect, thread suspended (wait-for-debugger)
    2. JDWP: InvokeMethod -> System.loadLibrary("frida-gadget")
        - (this call blocks/doesn't return yet - gadget is now
           listening on 27042, but paused waiting for a Frida client)
    3. adb forward tcp:27042 tcp:27042
    4. Frida: device.attach("Gadget")  ->  install hooks, load script
    5. Frida: device.resume("Gadget")
        - (gadget's internal block releases; loadLibrary() finally
           returns; your step-2 JDWP InvokeMethod reply arrives)
    6. JDWP: resume the VM/thread (let the app actually proceed past
        the original wait-for-debugger point into onCreate, etc.)
    """
    ...
    # Alternative to breakpoint.
    #await dbg.method_entry_loadlibrary("frida-gadget")
